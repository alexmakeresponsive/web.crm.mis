...
// ts-node ./app_modules/src/model/service/migration/2020-02-26-6/migration.ts
...
connection.beginTransaction(function(err) {
    ...
    connection.query(`
        ...
        function insert(user:any) {
            ...
            connection.query(`
                    INSERT INTO service (id_user, id_service, list_user_role)
                    VALUES ('${user.id_user}', '${user.id_service}', '${user.list_user_role}')
                    `, function (error, results, fields) {
                if (error) {
                    return connection.rollback(function() {
                        throw error;
